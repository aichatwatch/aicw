...
/**
 * Create linkTypes array by grouping links by their linkType code
 */
function createLinkTypesFromLinks(links: any[]): any[] {
  const typeMap = new Map<string, any>();

  for (const link of links) {
    const code = link.linkType || DEFAULT_OTHER_LINK_TYPE_SHORT_NAME;

    if (!typeMap.has(code)) {
      typeMap.set(code, {
        type: 'linkType',
        code,
        value: LINK_TYPE_NAMES[code] || DEFAULT_OTHER_LINK_TYPE_LONG_NAME
      });
    }
  }
...
/**
 * Main function to generate linkTypes from classified links
 */
export async function generateLinkTypes(project: string, targetDate: string): Promise<void> {
  // Initialize logger
  await logger.initialize(import.meta.url, project);

  logger.info(`Generating linkTypes from classified links for project: ${project}`);
  ...
  let totalLinkTypesCreated = 0;

  for (const [index, question] of questions.entries()) {
    const currentIndex = index + 1;
    logger.updateProgress(currentIndex, `Generating linkTypes for ${question.folder}...`);
    ...
    try {
      ...
      // Check if links exist - CRITICAL: links are required to generate linkTypes
      if (!data.links || !Array.isArray(data.links) || data.links.length === 0) {
        throw createMissingDataError(question.folder, 'Links', CURRENT_MODULE_NAME, logger.currentActionName);
      }

      // Generate linkTypes from links
      data.linkTypes = createLinkTypesFromLinks(data.links);

      const linkTypeCount = data.linkTypes.length;
      totalLinkTypesCreated += linkTypeCount;
      ...
      logger.updateProgress(currentIndex, `${question.folder} - ✓ ${linkTypeCount} types from ${data.links.length} links`);
      logger.info(`Generated ${linkTypeCount} link types from ${data.links.length} links for ${question.folder}`);
  ...
  logger.addStat('LinkTypes Created', totalLinkTypesCreated);

  logger.info(`LinkTypes generation complete. Processed: ${processedCount}, LinkTypes: ${totalLinkTypesCreated}`);
...
async function main(): Promise<void> {
 ...
 await validateAndLoadProject(project);
  ...
  await generateLinkTypes(project, targetDate);
